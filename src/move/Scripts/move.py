...
def robot_next_move_callback(msg):
    ...
    if is_moving:
        rospy.logwarn("[move_sawyer] Already moving. Ignoring new command.")
        return

    user_input = int(msg.data)

    terminal_log(
        "MOVE NODE: RECEIVED MOVE COMMAND",
        f"Column received from /robot_next_move: {user_input}"
    )
    ...
    try:
        ...
        is_moving = False
        rospy.loginfo("[move_sawyer] Ready for next /robot_next_move command.")
