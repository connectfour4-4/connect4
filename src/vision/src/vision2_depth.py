...
class VisionNode:
    def __init__(self):
        ...
        self.vote_frames = int(rospy.get_param("~vote_frames", 15))
        self.vote_required = int(rospy.get_param("~vote_required", 11))
        ...
        rospy.loginfo("[vision_node] RealSense stable detection enabled.")
        rospy.loginfo("[vision_node] Publishes /board_state only after stable detection.")
    ...
    def filter_board_with_votes(self, board):
        ...
        filtered = np.zeros((ROWS, COLS), dtype=np.int8)
    ...
    def update_stable_board(self, board):
        ...
        if not np.array_equal(board, self.last_candidate_board):
            self.last_candidate_board = board.copy()
            self.last_candidate_time = now
            rospy.loginfo("[vision_node] Filtered board changed. Waiting for 4s stability...")
            return
        ...
        if self.last_published_board is not None:
            if np.array_equal(board, self.last_published_board):
                return
    ...
    def publish_board(self, board):
        ...
        rospy.loginfo("[vision_node] Published stable board:")
        rospy.loginfo("\n" + str(board))
    ...
    def detect_board_and_overlay(self, frame):
        ...
        cv2.putText(out, "RealSense robust mode: vote + 4s stable publish",
